//T.C- (V + E) where V represents vertices and E represents edges, for dijstra algorithm same S.C as well
typedef long long ll;
    ...
    vector<bool> findAnswer(int n, vector<vector<int>>& edges) {
        ...
        for(int i = 0; i < E; i++){
              ...
              ll distFromSrc = fromSrc[u], distFromDest = fromDest[v];
              if(distFromSrc + w + distFromDest == fromSrc[n-1]) result[i] = true;

              distFromSrc = fromSrc[v], distFromDest = fromDest[u];
              if(distFromSrc + w + distFromDest == fromSrc[n-1]) result[i] = true;
        }
        return result;
    }